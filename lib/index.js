...
module.exports = co.wrap(function * (name, opts = {}) {
  ...
  const config = {
    ...
    resolve: {
      mainFields: ['browser'].concat(opts.es6 ? ['module', 'main'] : ['main']),
      modules: [
    ...
    module: {
      rules: [{
        ...
        include(filepath) {
          ...
          let pkg
          const manifest = path.resolve(filepath.replace(MODULE_DIR, '$1'), 'package.json')
          ...
          return Boolean(pkg.modules || pkg['jsnext:main'])
        }
      }]
    }
  }
