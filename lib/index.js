...
module.exports = co.wrap(function * (name, opts = {}) {
    ...
    module: {
      rules: [{
        ...
        include(filepath) {
          ...
          return opts.es6 && (isDependency || isCwd)
        }
      }]
    }
  }
  ...
  if (isCwd) {
    ...
    yield Promise.all(parsed.map(p => {
      return readPkg(path.join(outDir, 'node_modules', p.name))
        .then(pkg => {
          config.externals = config.externals
            .concat(pkg.peerDependencies ? Object.keys(pkg.peerDependencies) : [])
        })
  ...
  if (opts.externals) {
    ...
    config.externals = config.externals.concat(externals.map(v => typeof v === 'string' ? new RegExp(`^${v}$`) : v))
  }
